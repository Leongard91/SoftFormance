...
def following(request):
    ...
    for i in follow_to:
        for post in i.posted_posts.all():
            ...
            except: continue # If two posts with identical timestamp or change to pk
    
    for my_post in current_user.posted_posts.all():
        try: unsorted_d[my_post.timestamp] = my_post
        except: continue # If two posts with identical timestamp or change to pk
